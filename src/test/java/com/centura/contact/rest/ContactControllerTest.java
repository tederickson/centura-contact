...
public class ContactControllerTest {
	...
	public void testGetContactByIdVersion1InvalidId() throws Exception {
		MockHttpServletRequestBuilder builder = get("/v1/contact/-123456").contentType(MediaType.APPLICATION_JSON);
		mvc.perform(builder).andExpect(status().is4xxClientError());
	}

	@Test
	public void testGetContactByIdVersion1MissingId() throws Exception {
		MockHttpServletRequestBuilder builder = get("/v1/contact").contentType(MediaType.APPLICATION_JSON);
		mvc.perform(builder).andExpect(status().is4xxClientError());
	}
	...
	public void testGetContactByIdVersion1() throws Exception {
		...
		MockHttpServletRequestBuilder builder = get("/v1/contact/" + DEFAULT_ID).contentType(MediaType.APPLICATION_JSON);
	...
	public void testGetContactsVersion1NoResults() throws Exception {
		MockHttpServletRequestBuilder builder = get("/v1/contacts").contentType(MediaType.APPLICATION_JSON);
		MvcResult result = mvc.perform(builder).andExpect(status().isOk()).andReturn();
		String json = result.getResponse().getContentAsString();
